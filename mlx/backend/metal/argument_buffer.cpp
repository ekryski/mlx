...
ArgumentBuffer::ArgumentBuffer(Device& d, std::vector<Slot> slots)
  ...
  if (!dev) {
    throw std::runtime_error(
        "[metal::ArgumentBuffer] Metal device unavailable");
  }
  buffer_ = NS::TransferPtr(
      dev->newBuffer(size_, MTL::ResourceStorageModeShared));
  if (!buffer_) {
    throw std::runtime_error(
        "[metal::ArgumentBuffer] failed to allocate shared buffer of "
        + std::to_string(size_) + " bytes");
  }
  // Zero the contents so callers that never write to a slot still
  // see defined values on the GPU.
  std::memset(buffer_->contents(), 0, size_);
}

ArgumentBuffer::~ArgumentBuffer() = default;
